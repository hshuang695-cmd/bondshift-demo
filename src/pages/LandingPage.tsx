...
export default function LandingPage() {
  ...
  return (
    <main className="relative min-h-[100dvh] w-full overflow-hidden bg-[#fff9fb] text-text-primary">
      ...
      <header className="relative z-10 mx-auto flex w-full max-w-6xl items-center justify-between px-5 py-5 sm:px-8 sm:py-7">
        <Link to="/" className="flex items-center gap-2.5" aria-label="BONDSHIFT 首页">
      ...
      <section className="relative z-10 mx-auto grid w-full max-w-6xl items-center gap-12 px-5 pb-20 pt-10 sm:px-8 sm:pt-16 lg:grid-cols-[1.02fr_0.98fr] lg:gap-20 lg:pb-28 lg:pt-20">
        ...
        >
          ...
          <p className="mt-6 max-w-lg text-base leading-8 text-text-secondary sm:text-lg sm:leading-9">
          ...
          <div className="mt-8">
            <Link
              ...
              className="group inline-flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl gradient-brand px-7 py-4 text-base font-bold text-white shadow-[0_16px_36px_rgba(232,84,124,0.28)] transition-transform active:scale-[0.98] sm:w-auto"
            >
              开始陪伴测试
              <ArrowRight size={19} className="transition-transform group-hover:translate-x-1" />
            </Link>
            <p className="mt-3 text-center text-xs text-text-tertiary sm:text-left">
              无需注册 · 约 3 分钟 · 免费体验
            </p>
          </div>
        </motion.div>
              ...
              return (
                <div key={step.title} className="rounded-[1.75rem] border border-surface-200 bg-white p-6 shadow-[0_10px_30px_rgba(90,53,70,0.05)]">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-black text-brand-200">0{index + 1}</span>
                  </div>
      ...
      <section className="relative z-10 mx-auto grid w-full max-w-6xl gap-8 px-5 py-20 sm:px-8 sm:py-24 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
        ...
        <div className="rounded-[1.75rem] border border-accent-100/70 bg-white/75 p-6 shadow-[0_16px_40px_rgba(67,77,160,0.07)] backdrop-blur-sm sm:p-8">
          <ul className="grid gap-4 text-sm leading-6 text-text-secondary sm:grid-cols-2">
            <li className="flex gap-3"><span className="font-bold text-brand-500">01</span><span>首次体验无需注册。</span></li>
            <li className="flex gap-3"><span className="font-bold text-brand-500">02</span><span>聊天数据默认保存在当前浏览器。</span></li>
            <li className="flex gap-3"><span className="font-bold text-brand-500">03</span><span>测试分析不记录完整聊天原文。</span></li>
            <li className="flex gap-3"><span className="font-bold text-brand-500">04</span><span>当前为功能测试版，部分回复使用安全降级内容。</span></li>
          </ul>
        </div>
      </section>
      ...
      <section className="relative z-10 px-5 pb-20 sm:px-8 sm:pb-24">
        <div className="mx-auto max-w-6xl overflow-hidden rounded-[2rem] bg-[#302c31] px-6 py-10 text-center text-white shadow-[0_24px_70px_rgba(48,44,49,0.18)] sm:px-10 sm:py-14">
          ...
          <Link
            to="/setup"
            onClick={handleExperienceStart}
            className="group mt-7 inline-flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-white px-7 py-4 text-base font-bold text-[#302c31] transition-transform active:scale-[0.98] sm:w-auto"
          >
            开始陪伴测试
      ...
      <footer className="relative z-10 border-t border-white/80 px-5 py-7 text-center text-xs text-text-tertiary sm:px-8">
